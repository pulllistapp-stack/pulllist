...
import { submitSetReport, type SetReportCategory } from "@/lib/auth";
...
const CATEGORIES: {
...
}[] = [
  {
    value: "missing_cards",
    label: "Missing cards",
    hint: "Card rows are absent from this set",
    icon: AlertTriangle,
  },
  {
    value: "wrong_images",
    label: "Wrong images",
    hint: "Images are broken or don't match the cards",
    icon: ImageIcon,
  },
  {
    value: "wrong_metadata",
    label: "Wrong info",
    hint: "Set name, logo, release date, or total is off",
    icon: Info,
  },
  {
    value: "other",
...
/** "Report an issue" modal on set detail pages. Same shape as
 *  CardReportModal, but the categories are set-scoped: missing cards,
 *  wrong images, wrong metadata, other. */
export function SetReportModal({ setId, setName, onClose }: Props) {
  const [category, setCategory] = useState<SetReportCategory | null>(null);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="set-report-title"
    >
      <div
