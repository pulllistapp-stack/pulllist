"use client";

import Image from "next/image";
import Link from "next/link";

import type { Card } from "@/lib/api";

import { useCollection } from "./CollectionProvider";
import { PriceBadge } from "./PriceBadge";
import { RarityChip } from "./RarityChip";

type Props = {
  card: Card;
  priority?: boolean;
};

export function CardThumb({ card, priority = false }: Props) {
  const { has } = useCollection();
  const owned = has(card.id);

  return (
    <Link
      href={`/cards/${card.id}`}
      className={`group relative flex flex-col rounded-card border p-2 transition-colors ${
        owned
          ? "bg-accent-green/5 border-accent-green/30 hover:border-accent-green/60"
          : "bg-bg-surface border-border hover:border-accent-yellow/40"
      }`}
    >
      {owned && (
        <span
          className="absolute top-1 right-1 z-10 inline-flex items-center justify-center w-5 h-5 rounded-full bg-accent-green text-bg text-xs font-bold"
          title="In your collection"
        >
          ✓
        </span>
      )}

      <div className="relative aspect-[245/342] w-full overflow-hidden rounded-md bg-bg">
        {card.image_small ? (
          <Image
            src={card.image_small}
            alt={card.name}
            fill
            sizes="(max-width: 768px) 33vw, 200px"
            className="object-contain group-hover:scale-[1.02] transition-transform"
            unoptimized
            priority={priority}
            loading={priority ? "eager" : "lazy"}
          />
        ) : (
          <div className="flex h-full items-center justify-center text-text-tertiary text-xs">
            no image
          </div>
        )}
      </div>

      <div className="mt-2 px-1 flex flex-col gap-1">
        <div className="flex items-center justify-between gap-2">
          <span className="text-xs font-mono text-text-tertiary">
            #{card.number ?? "—"}
          </span>
          <PriceBadge price={card.market_price_usd} />
        </div>
        <div className="text-sm font-medium truncate" title={card.name}>
          {card.name}
        </div>
        <RarityChip rarity={card.rarity} />
      </div>
    </Link>
  );
}
