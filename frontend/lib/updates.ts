...
export const UPDATES: UpdateEntry[] = [
  // ── 2026-07-24 ─────────────────────────────────────────────────
  {
    date: "2026-07-24",
    emoji: "🎴",
    kr: "**KR 프로모 chase-tier 13장 자동 소싱 파이프라인 (eBay + Naver)** — 어제 tcgbox 백필 이후 아직 이미지 없는 XY/BW/base era 커버 위해 eBay Browse + Naver 이미지 검색 이중 파이프라인 구축. 필터: (1) aspect ratio 0.60-0.82 (포켓몬 카드 63/88 세로 프레임), (2) 이미지 중앙 40x40 픽셀 평균 RGB 로 **카드 뒷면 감지** (Naver 셀러들이 뒷면을 대표사진으로 자주 올려서 필요), (3) alt/title 에 `NNN/CODE` 정확 매치 요구, (4) `일어판/영문판/JUMBO/LOT/BUNDLE` 등 BAD 키워드 제외. eBay Sold + Active 두 번 시도 후 Naver 폴백. `shop1.phinf.naver.net` (Naver 레거시 CDN, HTTPS 미지원) 은 이미 whitelisted 된 `images.weserv.nl` 프록시로 자동 wrap. 결과: **chase-tier 20장 중 13장 정확 매치** (eBay 10 + Naver 3). 나머지 7장은 vintage 프로모 (2011-2013) 라 KR 국내/eBay 국제 시장 모두에서 현재 매물 없음. `backend/scripts/backfill_kr_promo_chase_images.py` 프로덕션 스크립트 승격 — TARGETS 리스트 확장하거나 `--card` 로 단일 카드 재시도 가능.",
    en: "**KR promo chase-tier — 13 cards auto-sourced via eBay + Naver pipeline** — after yesterday's tcgbox backfill, the XY/BW/base era cards were still uncovered so wired up an eBay Browse + Naver Image Search dual pipeline. Filters: (1) aspect ratio 0.60-0.82 (Pokemon card 63/88 portrait frame), (2) center-40×40-pixel avg RGB check to detect **card BACKS** (Naver sellers frequently upload backs as their primary thumbnail — needed to reject them), (3) alt/title must contain exact `NNN/CODE` match, (4) exclusion of BAD keywords like `Japanese/English/JUMBO/LOT/BUNDLE`. eBay Sold + Active tried first, Naver fallback. Legacy `shop1.phinf.naver.net` (Naver CDN, HTTPS-not-supported) URLs auto-wrapped through the already-whitelisted `images.weserv.nl` proxy. Result: **13/20 chase-tier cards matched** (eBay 10 + Naver 3). Remaining 7 vintage promos (2011-2013) have no current listings on either KR retail or eBay international. Promoted to `backend/scripts/backfill_kr_promo_chase_images.py` — TARGETS list is expandable or `--card` for single-card retry.",
  },
  {
    date: "2026-07-24",
    emoji: "🇰🇷",
  ...
  // ── 2026-07-22 ─────────────────────────────────────────────────
  {
    date: "2026-07-22",
    emoji: "🖼️",
    kr: "**KR 프로모 세트 4개 이미지 + 아트워크 백필 (collectory.cc)** — 어제 namu 에서 텍스트로만 임포트했던 KR 프로모 세트들 중 XY / SM / 소드실드 / SV 4개 era 를 collectory.cc (기존 KR 카탈로그 소스) 데이터로 교체. 카드 이미지 + 세트 로고 + KR 이름 모두 collectory CDN 에서 직접 서빙. 카드 수 대폭 확장: XY 191→**216**, SM 196→**251**, 소드실드 191→**305**, SV 26→**378** = 604 → **1150 카드** (+546). 이제 `/sets/ko-p-xy` 같은 페이지 열면 예전처럼 회색 placeholder 대신 실제 카드 아트가 뜸. 번호 체계는 collectory 기준으로 통일 (예전 XY 는 namu 의 `001/XY-P` 였는데 collectory 는 `XY01` — 인터내셔널 Black Star Promo 표기). 슬래시 (`/`) 는 `_` 로 sanitize (2026-07-20 slash-in-ID 버그 재발 방지). 남은 3개 era (초대 22 · BW 72 · MEGA 50 = 144 카드) 는 collectory 에 없어서 namu-only 상태 유지 (이미지 없음). 볼트/위시리스트 참조 0 rows 확인 후 안전하게 교체 (friends-beta 라 어차피 담은 사람 없었음).",
    en: "**KR promo images + artwork backfilled for 4 sets via collectory.cc** — the KR promos we imported yesterday from namu.wiki as a text-only catalog now get real card scans + set logos + KR native names for the four eras collectory covers (XY / SM / SS / SV). Card counts also grew substantially since collectory's promo grouping is broader than namu's: XY 191→**216**, SM 196→**251**, Sword & Shield 191→**305**, SV 26→**378** = 604 → **1150 cards** (+546). Set pages under `/sets/ko-p-xy` etc. now render actual artwork instead of the grey NO-ARTWORK placeholder. Card numbers follow collectory's international Black Star Promo convention (`XY01` / `SM01` / `SWSH001` / `001/SV-P`) rather than the JP-derived namu format (`001/XY-P`); slashes in card ids are sanitized to `_` to match the 2026-07-20 slash-rename fix. The remaining 3 eras (base 22 · BW 72 · MEGA 50 = 144 cards) stay on namu-only data since collectory doesn't group those as promos. Vault + wishlist references were zero at replacement time (friends-beta, nobody had added these yet), so the wipe-and-replace was safe.",
  },
  {
    date: "2026-07-22",
    emoji: "🇰🇷",
    kr: "**KR 프로모 카탈로그 nam.wiki 에서 임포트 — 새 세트 7개 + 카드 748장** — 지금까지 KR 프로모는 25주년 프로모팩 2021 (`ko-c-5f2269d335`) 하나만 DB 에 있었음. 나머지 초대 시리즈부터 최신 MEGA 프로모까지 KR 컬렉터가 자기 볼트에 담을 방법이 없었음. Playwright 로 `namu.wiki/w/포켓몬 카드 게임/한국 프로모 카드 일람` 스크레이핑 → 7개 시대별 테이블 파싱 → 7개 세트 신규 (`ko-p-base` / `ko-p-bw` / `ko-p-xy` / `ko-p-sm` / `ko-p-ss` / `ko-p-sv` / `ko-p-mega`) + 카드 **748장** 업서트. 세트 별 카드수: 초대 22 · BW 72 · XY 191 · SM 196 · 소드실드 191 · SV 26 · MEGA 50. Rowspan-aware 그리드 파서로 namu 특유의 병합 셀 (같은 이벤트로 배포된 카드들이 한 셀로 묶여있는 것) 정확히 분해. 이제 KR 유저가 예를 들어 '루기아 001 PROMO' 나 '치코리타 M-P' 를 검색해서 벌트에 담을 수 있음. **이미지는 이번엔 없음** — namu 리스트 페이지엔 카드 썸네일이 인라인으로 없고 개별 카드 문서로 들어가야 있어서, 이미지 소싱은 별도 트랙 (tcgdex KR / pokemon.com KR / 카드별 namu 문서 스크랩 중 하나) 으로 뺌.",
    en: "**KR promo catalog imported from namu.wiki — 7 new sets, 748 cards** — until now the DB carried a single KR promo set (25주년 프로모팩 2021, `ko-c-5f2269d335`); every other KR promo card from the base era through MEGA had no catalog entry, so a KR collector had no way to add a '루기아 001 PROMO' or '치코리타 M-P' to their vault. Playwright-scraped `namu.wiki/w/포켓몬 카드 게임/한국 프로모 카드 일람`, parsed the 7 era tables, and upserted 7 new sets + **748 cards**: base 22 · BW 72 · XY 191 · SM 196 · 소드실드 191 · SV 26 · MEGA 50. Set ids follow the `ko-p-{era}` convention (`ko-p-base` / `ko-p-bw` / `ko-p-xy` / `ko-p-sm` / `ko-p-ss` / `ko-p-sv` / `ko-p-mega`), one bucket per H2 era matching how JP promos are already grouped. Rowspan-aware grid parser handles namu's heavy cell-merging (SV era's `스타터 세트 ex 동봉 프로모팩` covers a dozen rows) so no card gets dropped from a shared-event group. **No images this pass** — the '일람' listing page has zero inline card thumbnails; images live only on individual card articles, so image sourcing gets its own follow-up track (tcgdex KR / pokemon.com KR / per-card namu article scrape).",
  },
  {
    date: "2026-07-22",
    emoji: "🏆",
  ...
  // ── 2026-07-20 ─────────────────────────────────────────────────
  {
    date: "2026-07-20",
    emoji: "☁️",
    kr: "**모든 카드 이미지 74K장 R2 CDN 으로 이관 + 슬래시 카드 ID 버그 픽스**. (1) 이관: 5개 로케일 전체 카드 이미지 (EN 20,834 + JA 20,894 + KO 17,953 + CN 11,743 + TW 2,825 = **74,249 카드**) 를 Cloudflare R2 (pulllist-setslogosbackup 버킷) 로 업로드 완료. 이제 카드 상세페이지의 대형 이미지 + 카탈로그 썸네일 다 우리 CDN edge cache 로 서빙 → 로딩 훨씬 스냅. 기존 8개+ 외부 CDN (pokemontcg.io / limitless / bulbagarden / collectory / asia.pokemon-card.com / weserv 등) 의존성 제거. Cache-Control immutable 로 브라우저 캐시도 최대치. (2) 버그 픽스: 콜렉토리 임포트된 KR/CN 카드 29,021 개가 ID 에 `/` 포함 (예: `ko-c-*-234/208` — 234/208 은 카드 번호). Next.js `/cards/[cardId]` 라우팅이 slash 를 path separator 로 파싱 → 카드 상세 페이지가 404. `/` → `_` 로 일괄 rename (`ko-c-*-234_208`). 트랜잭션 안전. FK 참조 0 rows 확인 후 straight UPDATE. R2 이미지 파일명도 sanitize 돼있어서 매칭 완벽.",
    en: "**All 74K card images migrated to R2 CDN + slash-in-ID routing bug fixed**. (1) Migration: every catalog card image across 5 locales (EN 20,834 + JA 20,894 + KO 17,953 + CN 11,743 + TW 2,825 = **74,249 cards**) uploaded to Cloudflare R2 (pulllist-setslogosbackup bucket). Card detail pages and browser tiles now serve from our own CF edge cache — snappy loads, no dependency on the 8+ external CDNs (pokemontcg.io / limitless / bulbagarden / collectory / asia.pokemon-card.com / weserv / etc). Cache-Control immutable lets the browser cache go maximum too. (2) Bug fix: 29,021 collectory-imported KR/CN cards had `/` inside their IDs (e.g. `ko-c-*-234/208`, where `234/208` is the printed card number). Next.js `/cards/[cardId]` routing parsed the slash as a path separator, so card detail pages 404'd. Renamed `/` → `_` in a single transaction (`ko-c-*-234_208`). Verified 0 FK rows referenced the old IDs before touching the PK. R2 image filenames were already sanitized during backup so the URL swap matched cleanly.",
  },
  {
    date: "2026-07-20",
    emoji: "🏆",
  ...
  // ── 2026-07-19 ─────────────────────────────────────────────────
  {
    date: "2026-07-19",
    emoji: "☁️",
    kr: "**모든 세트 로고를 Cloudflare R2 자체 CDN 으로 이관** — 그동안 EN/JP/KR/CN/TW 세트 로고 842개가 8개+ 외부 CDN 에 흩어져 있었음 (koca.shop / hobbyxstore / weserv 프록시 / naver phinf / limitless / bulbagarden / pokemon.com CDN / TCGdex 등). 각 호스트가 언제든 410 될 위험 (pokemonkorea.co.kr 은 이번 분기에 실제로 죽어서 우리 링크 몇 개 깨짐). 이제는 우리가 통제하는 R2 버킷 (pulllist-setslogosbackup) 에 837개 다 미러링 + DB 의 logo_url 컬럼도 R2 pub URL 로 일괄 스왑. 로컬 백업 폴더 (backend/data/logo_backup/) → boto3 로 R2 업로드 → DB UPDATE 트랜잭션. 응답 시간 ~340ms 하드 CDN edge cache, Cache-Control immutable 로 next/image 도 aggressive cache. 무료 티어 (10GB 스토리지 / 1M ops월 / 10M read월) 안에서 완전 여유 (우리 사용 0.15GB). 5개 남은 외부 URL 은 upstream 소스 이미 죽어서 백업 못한 KR 세트들 — 재스크레이핑 별도 트랙 필요.",
    en: "**Migrated every set logo to Cloudflare R2 self-hosted CDN** — the 842 set logos across EN/JP/KR/CN/TW were scattered across 8+ external CDNs (koca.shop / hobbyxstore / weserv proxy / naver phinf / limitless / bulbagarden / pokemon.com CDN / TCGdex). Each host could 410 at any time (pokemonkorea.co.kr already did this quarter, breaking some tiles). 837 are now mirrored to our own R2 bucket (pulllist-setslogosbackup) and the sets.logo_url column swapped to pub-*.r2.dev URLs in one transaction. Pipeline: local backup folder → boto3 → R2 upload → DB UPDATE. Response time ~340ms edge cache with Cache-Control: immutable so next/image caches aggressively too. Well within R2's free tier (10GB storage / 1M writes-per-month / 10M reads-per-month) — we use 0.15GB. 5 sets still point at external URLs because their upstream sources 410'd before we could back them up (KR-only) — separate re-scrape track needed.",
  },
  {
    date: "2026-07-19",
    emoji: "✅",
    kr: "**zh-tw 로고 100% 커버리지 달성** — koca.shop 자동 매칭 23/27 이후 남은 4개 처리. MBG 挑戰牌組「超級耿鬼ex」 는 LO 가 hobbyxstore.com URL 하나 직접 던져줘서 채웠고, 나머지 3개 (MJ New Trainer Journey / SVQP ex초階牌組 皮卡丘 / SVQL ex초階牌組 噴火龍) 는 koca 에도 없고 실물 사진 찾기 어려워서 세트 자체 삭제 (총 71 카드 이관 없이 CASCADE drop). 최종: **24/24 zh-tw 세트 100% 로고** (Mega 15 + SV 9), 3,654 카드. hobbyxstore.com next.config 화이트리스트 추가.",
    en: "**zh-tw logo coverage now 100%**. Automatic koca.shop matching landed 23/27; LO hand-supplied a hobbyxstore.com URL for the last legit set (MBG 挑戰牌組「超級耿鬼ex」). The remaining 3 (MJ New Trainer Journey, SVQP Pikachu ex starter, SVQL Charizard ex starter) had no clean product photos anywhere, so we dropped them from the catalog entirely (71 cards cascade-deleted, no rescue path). Final: 24/24 zh-tw sets with logos (15 Mega + 9 SV), 3,654 cards. hobbyxstore.com added to next.config remotePatterns.",
  },
  {
    date: "2026-07-19",
    emoji: "🎁",
    kr: "모바일 폴리시 3종. (1) 카드 상세 페이지 하단 sticky CTA 를 하나짜리 'Buy on TCGplayer' 에서 **TCGplayer / eBay 반반 버튼** 으로 분리. 각 버튼에 해당 마켓 최저가 표시, 유저가 어느 마켓 볼지 직접 선택. Safe-area-bottom 패딩 추가해서 아이폰 홈 인디케이터랑 안 겹침. (2) `/sets` 리전 탭 (USA / Japan / Korea / China / Taiwan) — 라벨 긴 게 뷰포트 오버플로해서 잘림. `overflow-x-auto` + `shrink-0` 조합으로 안쪽 스크롤. 스크롤바 숨김. (3) Bulk 스캔 안정성 threshold 완화. iPhone 카메라가 autofocus/AE 마이크로조정으로 프레임끼리 18-25 비트 드리프트 흔함. threshold 14 로는 절대 안정 안 잡혀서 발사 안 됐음. hamming ≤ **28** + steady tick 요구를 **1** 로 (즉시 발사). Gemini 콜 $0.0001 이라 조금 자주 발사해도 비용 무시할 수 있음.",
    en: "Three mobile polish fixes. (1) Card detail sticky CTA switched from a single 'Buy on TCGplayer' pill to **split TCGplayer / eBay buttons** — each shows its own market's low price and the user picks the marketplace instead of us picking the cheapest. Safe-area padding added so the buttons clear the iPhone home indicator. (2) /sets region tabs (USA / Japan / Korea / China (Simplified) / Taiwan (Traditional)) — the long labels overflowed the viewport and clipped. Container now scrolls internally with overflow-x-auto + shrink-0 on each tab, scrollbar hidden. (3) Bulk scan stability threshold loosened. iPhone rear cameras drift ~18-25 bits between frames even at rest (AF + AE micro-adjustments), so the previous threshold of 14 essentially never landed and the vision call never fired. Threshold now hamming ≤ **28** and we fire on the first stable tick instead of requiring two consecutive. Gemini is cheap enough that occasionally firing during a semi-motion frame is fine — the same-card cooldown prevents duplicate charges anyway.",
  },
  {
    date: "2026-07-19",
    emoji: "🖼️",
    kr: "**zh-tw 세트 로고 27/27 중 23개 커버** (Bing 롤백 후 재도전) — LO 가 koca.shop (대만 포켓몬 카드 리테일러) URL 하나 넘겨줌. 사이트에 planet 개념으로 지역별 Pokemon 카탈로그 분리돼있고, `/api/planets/pokemoncard_tw/series?limit=100` 로 82세트 정식 리스트 뽑을 수 있음 (limit=200+ 은 서버 500 뱉음). subprocess 로 curl 감싸서 (urllib 은 HTTP/1.1 이라 edge 500 리턴, curl HTTP/2 는 OK) JSON 가져오고 name normalization + SequenceMatcher fuzzy match 로 우리 27세트에 페어링. Bing 때 실수 (substring 필터 없음 → 噴火龍 이 超級噴火龍Xex 매치되는 등) 방지 위해 substring 매칭은 짧은/긴 길이 비율 ≥0.75 인 경우만 통과, 최종 스코어 0.90 이상만 허용. 결과 이미지는 api.koca.shop CDN (실물 팩 사진 검증된 것). 매칭 실패 4세트 (超級耿鬼ex / New Trainer Journey / ex초階牌組 皮卡丘 / ex초階牌組 噴火龍) 는 koca 카탈로그에 아예 없어서 LO 수동 보급 필요.",
    en: "**zh-tw set logos: 23/27 covered** (retry after the Bing rollback disaster). LO pointed to koca.shop, a Taiwan Pokemon retailer. Their site organizes locales under 'planets' and exposes `/api/planets/pokemoncard_tw/series?limit=100` which returns the curated 82-series zh-tw catalog (limit=200+ blows up the server). Fetched via subprocess curl (urllib's HTTP/1.1 hits a 500 at the edge; curl's HTTP/2 gets a clean 200), then name-normalized both sides and paired via SequenceMatcher. Learning from the Bing misfire: substring matches now require the shorter-vs-longer length ratio to be ≥0.75 (otherwise 噴火龍 would spuriously match 超級噴火龍Xex), and the overall confidence threshold is bumped to 0.90. Images stored are koca's CDN URLs (api.koca.shop/images/{uuid}.jpeg) — real, retailer-verified pack shots. 4 sets missing from the koca catalog (Gengar Challenge, New Trainer Journey, ex-starter Pikachu, ex-starter Charizard) still need manual URLs from LO.",
  },
  {
    date: "2026-07-19",
    emoji: "🔄",
