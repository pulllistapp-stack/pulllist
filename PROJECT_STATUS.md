...
39. **KR promo chase-tier — eBay + Naver dual-source pipeline** (2026-07-24) — §11.38 landed 66 tcgbox scans concentrated in recent SV/MEGA eras; XY/BW/base were still at 0% because tcgbox is a retail marketplace that doesn't stock vintage inventory. This pass targets the **chase-tier subset** (20 cards LO surfaced from a Naver-based market-activity probe scored $20+ USD by knowledge — Mewtwo EX, Charizard EX, Rayquaza, Lugia, Blastoise EX, Venusaur EX, M Salamence EX, Reshiram EX, Zekrom EX, base-era Lugia/Ho-Oh/Snorlax LV.X, etc.) via a dual-source Playwright pipeline. **eBay path** — Sold + Active search with `{english name} Korean {code} {num}`; on match, upgrade the search-result thumb URL from `s-l500` → `s-l1600` for the full-res version. **Naver fallback** — image search `{kr name} {num}/{code} 포켓몬카드`, decode `search.pstatic.net/common/?src=` proxy URLs into the underlying phinf CDN. Both paths share the same quality gates: (a) aspect ratio 0.60–0.82 (Pokemon card portrait 63/88=0.716), (b) min width 300px, (c) **card-back detection** — download the image and sample the center 40×40 region avg RGB; Naver sellers frequently upload the pokéball-blue back as their primary thumbnail (LO caught this on the first pass), and a signature of `avg B > 130 AND avg R > 80 AND avg G < 130` cleanly rejects those, (d) exact `NNN/CODE` substring must appear in title/alt, (e) foreign-language rejection (일어판/영문판/Japanese) and BAD keyword blocklist (JUMBO/OVERSIZED/LOT/BUNDLE/SLEEVE/BOX/etc.). Legacy `shop1.phinf.naver.net` doesn't support HTTPS (SSL cert mismatch on the hostname) so Naver-sourced HTTP URLs get auto-wrapped through the already-whitelisted `images.weserv.nl` proxy — Next.js `<Image>` optimizer only accepts HTTPS `remotePatterns`, and weserv does the protocol upgrade transparently. Result: **13/20 chase-tier cards filled** — eBay hit 10 (Charizard 147/XY-P 20th Anniv Korean $105, Rayquaza 166/XY-P $67, Venusaur EX 168/XY-P BREAK $69, M Salamence EX 178/XY-P Korean PSA 8 $72, Lugia 191/XY-P Blackstar $70, Reshiram EX 041/BW Pack Promo Korean $79, Zekrom EX 042/BW-P Special Pack Korean $78, base-era Lugia 001 HGSS $139, Ho-Oh 002 Black Star Promo $170, Snorlax Lv.X 006 PSA 4 $500), Naver hit 3 (Mewtwo EX 122/XY-P 티씨지헤븐, Blastoise EX 170/XY-P 티씨지헤븐, White Kyurem 060/BW 슈미카드). Remaining 7 (Mewtwo 188, Charizard 011, Magearna 183, Black Kyurem 059, Umbreon 071, Glaceon 068, Genesect 061) had **no exact-num+front listing on either source** — vintage 2011-2013 BW promos are rarely traded and current listings on the same pokemon are for different sets (e.g. Black Kyurem 059/bw exists on Naver but the 슈미카드 photo is a card back, filter correctly rejected). `backend/scripts/backfill_kr_promo_chase_images.py` ships the dual-source pipeline — TARGETS list is hardcoded for the 20 chase cards (KR name → English name lookup is per-card, and we don't yet have a translation table for the full 693 uncovered pool), `--dry-run` for no-write mode, `--ebay-only` / `--naver-only` to run one source at a time, `--card <id>` for single-card retry. Coverage after this pass: **79 / 772 (10.2%)** — 66 tcgbox + 13 chase-tier, distributed as SV 31 · MEGA 7 · SM 20 · SS 8 · XY 5 · BW 3 · base 5.

38. **KR promo cleanup + tcgbox.co.kr KR-native scan backfill** (2026-07-24) — §11.37's collectory backfill turned out to be wrong-headed. Cross-checked collectory's `region` field on the 4 imported sets: **SV-P was 209 CN + 169 JP prints, XY-P was 216 US Black Star Promos, SM/SS BSP were 251+305 US prints — zero KR-released cards** across all 1,150 rows. Collectory serves international (JP/EN/CN) promo catalogs with KR name overlays; the underlying card scans are the JP/EN/CN physical prints, not KR releases. Wipe: dropped all 1,150 collectory rows and reran `import_kr_promos_namuwiki.py` to restore the 748-card namu 「한국 프로모 카드 일람」-anchored catalog. Also SQL-fixed the inflated `set.total` back to actual counts (216→191, 251→196, 305→191, 378→26). New source hunt: probed pokemon.co.kr (defunct — every URL 404s with a gabia hosting-provider fallback), asia.pokemon-card.com/kr/ (returns 404), and namu.wiki per-card articles (redirect to generic Pokemon articles whose first ~50 images are Pokedex icons, not card scans — dedicated 「(포켓몬 카드 게임)」 articles exist for only ~10% of pokemon and even those catalog JP-set cards). LO then pointed at **tcgbox.co.kr** — a KR TCG retail marketplace. Sitemap-based scrape (5 gzipped chunks, ~8,200 product URLs total, 63 promo-slugged) → visit each product page → regex-extract the largest `/web/product/(original|big|medium)/` image URL → map era code (`sv-p`/`sm-p`/`s-p`/`m-p`/`xy-p`/`bw-p`) to `ko-p-{era}` and match by number. Ran end-to-end: **42 existing namu rows updated + 24 new rows inserted** for KR promos namu missed (all recent SV, e.g. #042 스코빌런ex, #048 달이 빛나는 언덕, #197 기본 전기에너지 비크티니쟁탈전). Final coverage: 66/772 cards (8.5%) with real KR-language scans, concentrated in the recent-market ranges tcgbox actually stocks — SV 31/50 (62%), MEGA 7/50 (14%), SM 20/196 (10%), SS 8/191 (4%), XY/BW/base 0/285 (0%). Images serve directly from `tcgbox.co.kr` URLs (the hostname was already on the next/image allowlist from earlier KR set-logo work) — a later batch can mirror into R2 alongside the other 74k images. `backend/scripts/import_tcgbox_kr_promo_images.py` ships the pipeline (sitemap fetch + slug parser + image extraction + upsert) so a monthly rerun captures new tcgbox stock. `--dry-run` for txn rollback, `--skip-insert` to only refresh existing rows without adding new numbers. Doesn't ship the earlier `import_kr_promos_collectory.py` for the promo case — that script's UUID targets were the international sets, so pointing it at ko-p-* would just re-import the same bad data.

37. **KR promo artwork + logo backfill via collectory.cc** (2026-07-22) — §11.36's namu import gave us the text catalog but zero images, so the four biggest ko-p-* sets (xy / sm / ss / sv) rendered as walls of "NO ARTWORK" placeholders. `backend/scripts/import_kr_promos_collectory.py` reuses the existing `import_kr_from_collectory.py` pattern (collectory.cc — Korean fan-run archive, Cloudflare-fronted so we send a full ko-KR browser header set), maps our era codes to hand-verified collectory UUIDs (`bc2389e4` XY-P / `523eb016` SM BSP / `4a2071be` SWSH BSP / `305842f8` SV-P), wipes namu-imported cards under each target set, then upserts collectory's fuller dataset with `image_small`/`image_large` from `cdn_image_url` and set `logo_url` from the parent set's `image_url`. Card counts jumped 604 → 1,150 (+546 across the 4 eras): XY 191→**216**, SM 196→**251**, SS 191→**305**, SV 26→**378**. Card ids follow collectory's international Black Star Promo numbering (`XY01` / `SM01` / `SWSH001` / `001/SV-P`) with `/` → `_` sanitized to stay clear of the Next.js `[cardId]` slash-parsing trap the 2026-07-20 rename fixed. Two collectory API footguns worth remembering: `/sets/{uuid}` returns **405 Method Not Allowed** (only `/sets` list + `/sets/{uuid}/cards` nested route work — the script now caches the list on startup and looks up by uuid), and set-level `total`/`printed_total` on our end doesn't refresh unless we write it explicitly (initial run left the header displaying 191/216 because namu's stale `total` was still on the row; fixed by having `_replace_era` also set `row.total = len(coll_cards)`). Wipe-and-replace was safe here because `collection_items` and `wishlist_items` had **0 rows** referencing anything under `ko-p-*` at replacement time (friends-beta, nobody had added these yet). Remaining 3 eras (base 22 · bw 72 · mega 50 = 144 cards) stay on namu-only data — collectory doesn't carry a promo grouping for those. `--dry-run` for txn rollback, `--era xy|sm|ss|sv` for single-era reruns, `--skip-delete` for additive-only mode used to backfill just the `set.total` fields after the wipe already happened.
