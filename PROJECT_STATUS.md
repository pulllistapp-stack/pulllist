...
33. **DECK tile sealed_value backfill — coverage expansion setup** (2026-07-24) — after §32 landed JP price coverage on cards, the next gap surfaced on the /sets browser: of 118 JP DECK-type sets (starter decks, build boxes, trainer boxes, preconstructed decks), only 21 were displaying the new "Sealed value" pill. Root cause: the ingest_jp_sealed.py matcher routes TCGCSV groups → PullList set_ids via (1) `_GROUP_ID_OVERRIDES`, (2) `_TCGCSV_ALIASES`, (3) direct `abbr.lower() == set_id.lower()` fallback — the workflow itself is `workflow_dispatch`-only and hadn't run since recent alias/seed changes landed, so any newly-seeded target set (SVLS, CLL, PtA-*, JPP-M etc.) still had no attached products. Three-part push: (a) extended `seed_missing_jp_sealed_sets.py` with CLV (Classic Blastoise), CLF (Classic Venusaur), NPF2 (Neo Premium File 2), NPF3 (Neo Premium File 3) — all four are DECK-flavored stubs whose TCGCSV groups exist and carry real sealed SKUs but had no PullList target row. (b) Added groupId overrides `24002 → PtA-GF` and `24003 → PtA-LP` for the duplicate Pt Arceus LV.X Deck entries TCGCSV kept in parallel with 24054/24055 (same physical product, different TCGCSV group id) — without these, whichever ingest pass ran first attached to the ambiguous shared-abbr group and the other ended up in `unmatched`. (c) Added a `--report-gaps` diagnostic mode to `ingest_jp_sealed.py` — walks every unmatched TCGCSV group, fetches its product list, counts sealed SKUs via the shared `_is_sealed_product()` classifier (hoisted to module scope so ingest + report use the same rules), and logs a ranked table sorted by SKU count so LO can triage top-value alias/override/stub candidates first. Workflow `ingest-jp-sealed.yml` gains a `report_gaps` input; when true, the seeding steps are skipped and the run is diagnostic-only. Post-run expected: 21 → ~30-40 sets with `sealed_value_usd > 0` after re-triggering the workflow (aliases + overrides now bind their targets); remaining gaps enumerated in the report output — most are vintage empty-abbr groups (Movie Commemoration Sets, Emerald Gift Boxes, ADV-era Constructed Starter Decks, Aqua/Magma Deck Kits) that need either new stub sets or per-groupId overrides in a follow-up. The ~40 empty-abbr vintage groups + ~19 shared-abbr disambiguations (Pt/BK/BW1/DPt/sA/sC/DP1/DP3/DP4/DP5/L2/E/MG/MMB/svAM) are documented but deferred — LO's call which ones justify a stub.
